...
2: This problem occurs for all controls. The Bluetooth connection is often unreliable. While pairing and connecting the controller with a Bluetooth manager initially works, the controller eventually stops being recognized by applications like RetroArch and jstest after a few weeks. When this occurs, the controller is not identified in RetroPad Binds -> Port 1 Controls -> Device Index, showing "N/A (#1)" instead. This problem emerged after reinstalling the distribution, and using live mode was ineffective for the kernel. For developers: Creating a default virtual machine installation can be beneficial for cloning when you need to investigate this issue, as it avoids the need for repeated reinstallation. To enable Bluetooth functionality for a virtual machine, you just need to redirect the Bluetooth dongle to it. In virt-manager, start a virtual machine and navigate to the menu bar, then select Virtual Machine -> Redirect USB Device. To identify the correct device, you can unplug the dongle and plug it back in to observe it dynamically reappearing in the list.
